...
function modify(){
	var dataForm = $("form[name=billForm]").serialize();
	$.ajax({
		type:"GET",
...
$(function(){
	...
	$.ajax({
		...
		data:{method:"showProviderName"},//请求参数
	...
	var bid = location.search.substring(5);
	$.ajax({
		type:"GET",//请求类型
		url:path+"/jsp/bill.do",//请求的url
		data:{method:"view",bid:bid},//请求参数
		dataType:"json",//ajax接口（请求url）返回的数据类型
		success:function(data){//data：返回数据（json对象）
			...
			$("[name=isPayment][value="+data.isPayment+"]").attr("checked",'checked')
		}
	});
	/*
	 * 验证
